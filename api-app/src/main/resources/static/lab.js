...
async function startSearch() {
    ...
    try {
        const config = selectedSearchConfiguration(); const stops = selectedStopConditions(); const dataset = await materializeDataset();
        ...
        labState.searchRunId = run.searchRunId; labState.searchStartedAt = Date.now(); labState.stopConditions = stops; byId("cancel-search").disabled = false; byId("search-message").textContent = `Search ${run.searchRunId}`;
