...
final class JdkBinanceTransport implements BinanceTransport {

    private final HttpClient httpClient;
    private final Duration requestTimeout;

    JdkBinanceTransport(Duration connectTimeout, Duration requestTimeout) {
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new IllegalStateException(
                        "Binance returned HTTP " + response.statusCode());
            }
            return response.body();
        } catch (IOException exception) {
            throw new IllegalStateException("Binance HTTP request failed", exception);
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Binance HTTP request was interrupted", exception);
        }
    }
        ...
        WebSocket.Listener listener = new WebSocket.Listener() {
            ...
            public CompletionStage<?> onClose(WebSocket webSocket, int statusCode, String reason) {
                notifyDisconnected(new IllegalStateException(
                        "Binance WebSocket closed with status " + statusCode + ": " + reason));
                return null;
            }
